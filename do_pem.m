...
for i = 1 : min(length(t_start), length(t_end))
  if use_airspeed
    dat{i} = BuildIdDataRPYAirspeed(est, baro, u, t_start(i), t_end(i), dt, delay_ms);
  else
...
for i = 1 : length(merged_dat.OutputData)
  
  x0_dat{i} = merged_dat.OutputData{i}(1,:);
...
x0_dat_full{4} = [ x0_dat{1}(1)  x0_dat{2}(1) ];
x0_dat_full{5} = [ x0_dat{1}(2)  x0_dat{2}(2) ];
x0_dat_full{6} = [ x0_dat{1}(3)  x0_dat{2}(3) ];

%x0_dat_full{7} = [ x0_dat{1}(4)  x0_dat{2}(4) ];
x0_dat_full{7} = [ 10 10 ];%zeros( 1, length(merged_dat.OutputData));
